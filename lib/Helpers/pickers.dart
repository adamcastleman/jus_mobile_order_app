...
class Picker {
  time(BuildContext context) {
    if (PlatformUtils.isIOS() || PlatformUtils.isAndroid()) {
      return _timeMobileLayout(context);
    } else {
      return _timeWebLayout(context);
    }
  }

  date(BuildContext context, List<Map<String, dynamic>> scheduledList) {
    if (PlatformUtils.isIOS() || PlatformUtils.isAndroid()) {
      return _dateMobileLayout(context, scheduledList);
    } else {
      return _dateWebLayout(context, scheduledList);
    }
  }

  _timeMobileLayout(BuildContext context) {
    return ModalBottomSheet().partScreen(
      context: context,
      isScrollControlled: true,
      builder: (context) => _selectTimeWidget(),
    );
  }

  _timeWebLayout(BuildContext context) {
    return showDialog(
      context: context,
  ...
  _dateMobileLayout(
      BuildContext context, List<Map<String, dynamic>> scheduledList) {
    return ModalBottomSheet().partScreen(
      isScrollControlled: true,
      isDismissible: false,
      enableDrag: false,
      context: context,
      builder: (context) => _selectDateWidget(scheduledList),
    );
  }
    ...
    return showDialog(
      ...
      builder: (context) => Dialog(
        ...
        child: Padding(
          ...
          child: SizedBox(
            width: AppConstants.dialogWidth,
            child: _selectDateWidget(scheduledList),
          ),
        ),
      ),
    );
      ...
      builder: (context, ref, child) => Wrap(
        children: [
          ...
          ResponsiveLayout.isWeb(context)
              ? const Center(
                  child: Text('Scroll times with scroll wheel or trackpad'),
                )
              : const SizedBox(),
        ],
      ),
  ...
  Widget _selectDateWidget(List<Map<String, dynamic>> scheduledList) {
    return Consumer(
